...
class Shadow:
    ...
    def observe(self, condition):
        # Store the condition that would be printed
        self.captured_output.append(condition)
        # Call the original dumper's observe method
        self.dumper.observe(condition)
        # Return the condition for further use
        return condition
...
def main():
    ...
    # Create original dumper
    original_dumper = Dumper()
    # Create shadow wrapper
    dumper = Shadow(original_dumper)
    ...
    for rule in yara_file.rules:
        ...
        # Use the shadow dumper and capture the return value
        result = dumper.observe(rule.condition)
        # Now you can use 'result' as needed
    
    print(f"Captured result: {dumper}")
