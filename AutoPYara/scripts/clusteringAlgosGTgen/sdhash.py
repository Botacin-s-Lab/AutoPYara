...
def loadSimScores(path,thresholdVal):
    ...
    # Compute and print how many rows were dropped
    dropped = original_count - len(df)
    print(f"Number of dropped rows: {dropped}")
    return df
...
def main():
    threshold=30
    output_dir = "/usr/src/app/Dev/newtest/sdhash/"
    csv_name='merge'
    distance_matrix,file_paths=computeScan(savepath='/usr/src/app/Dev/newtest/sdhash/distance_30.npz',path='matchrun2',thresholdVal=threshold,pcd=False)
